...
void main (int argc, char *argv[])
{
  ...
  if (argc > 1)
    for (i=1; i<argc; i++)
    {
      if (strchr (argv[i], '.') != NULL) strcpy (gamefile, argv[i]);
      else
      {
        if (argv[i][0] == '/' || argv[i][0] == '-') strcpy (s, &argv[i][1]);
        else strcpy (s, argv[i]);
        strlwr (s);
        if (strstr_ext (s, "no|nointro")) fl_intro = 0;
        else if (strstr_ext (s, "vanilla|orig|original")) fl_vanilla = 1;
        else if (strcmp (s, "noautosave") == 0) fl_autosave = 0;
        else if (strstr_ext (s, "?|h|help")) prn_cmdhelp (argv[0]);
	else if (strcmp (s, "loadlast") == 0) fl_loadlast = 1;
	else if (strstr_ext (s, "new|newgame")) fl_newgame = 1;
        else
        {
  ...
  if (fl_loadlast) loadlast();
  if (strlen (gamefile) > 0) loadgame (gamefile);
  if (fl_newgame)
  {
    fl_intro = 0;
    new_game (1);
  ...
  do
  {
    ...
    switch (n)
    {
      case 1: new_game (1); break;
      case 2: prn_rule(); break;
      case 3: fl_vanilla = !fl_vanilla; break;
      case 4: fl_autosave = !fl_autosave; break;
      case 5: loadlast(); break;
    }
  }
  while (n != 6);
  clrscr();
